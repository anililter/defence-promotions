    ...
    <div className="min-h-screen bg-black">
      ...
      <section className="relative min-h-screen flex items-center overflow-hidden">
        {featuredEvent?.poster_url ? (
          <Image src={featuredEvent.poster_url} alt={featuredEvent.title} fill className="object-cover opacity-20" priority />
        ) : (
          <div className="absolute inset-0 bg-gradient-to-br from-red-950/30 via-black to-black" />
        )}
        <div className="absolute inset-0 bg-gradient-to-r from-black via-black/80 to-transparent" />
        <div className="absolute inset-0 opacity-5" style={{ backgroundImage: 'repeating-linear-gradient(0deg, #fff 0, #fff 1px, transparent 0, transparent 50%), repeating-linear-gradient(90deg, #fff 0, #fff 1px, transparent 0, transparent 50%)', backgroundSize: '60px 60px' }} />

        <div className="container-site relative z-10 pt-24 pb-12">
          <div className="max-w-2xl">
            <div className="section-label mb-4">{featuredEvent?.status === 'upcoming' ? '🔴 Yaklaşan Etkinlik' : 'Öne Çıkan Etkinlik'}</div>
            ...
            {featuredEvent?.subtitle && <p className="text-gray-400 text-lg mb-2">{featuredEvent.subtitle}</p>}

            <div className="flex flex-wrap gap-4 mb-6 text-sm text-gray-500">
              {featuredEvent?.event_date && (
                <span className="flex items-center gap-2">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#C8102E" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                  {formatDate(featuredEvent.event_date)}
                </span>
              )}
              {(featuredEvent?.venue || featuredEvent?.city) && (
                <span className="flex items-center gap-2">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#C8102E" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
            ...
            {mainFight && (
              <div className="mb-8 p-4 border border-red-900/30 bg-red-950/10 rounded-sm">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-white font-bold tracking-widest uppercase" style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '20px' }}>{mainFight.fighter1_name}</span>
                  <span className="text-red-600 font-bold" style={{ fontFamily: 'Bebas Neue, sans-serif' }}>VS</span>
                  <span className="text-white font-bold tracking-widest uppercase" style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '20px' }}>{mainFight.fighter2_name}</span>
                </div>
                {mainFight.weight_class && <div className="text-gray-600 text-xs mt-1">{mainFight.weight_class as string}</div>}
              </div>
            )}

            <div className="flex flex-wrap gap-4">
              {featuredEvent && <Link href={`/etkinlik/${featuredEvent.slug}`} className="btn-red">Etkinlik Detayı</Link>}
              {featuredEvent?.ticket_url && <a href={featuredEvent.ticket_url as string} target="_blank" rel="noopener noreferrer" className="btn-gold">Bilet Al</a>}
              {!featuredEvent && <Link href="/etkinlikler" className="btn-red">Tüm Etkinlikler</Link>}
            </div>
          </div>
        </div>
      ...
      {upcomingEvents.length > 1 && (
        <section className="py-20">
          <div className="container-site">
            ...
            <h2 className="section-title text-white mb-10">KAFES KAPISI <span className="text-red-600">AÇILIYOR</span></h2>
        ...
        <section className="py-20" style={{ backgroundColor: 'var(--color-dark)' }}>
          <div className="container-site">
            <div className="flex items-end justify-between mb-10">
              ...
              <Link href="/etkinlikler" className="btn-outline hidden md:flex">Tümünü Gör</Link>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      <section className="py-20 relative overflow-hidden">
        ...
        <div className="container-site relative z-10 text-center">
          ...
          <h2 className="section-title text-white mb-6" style={{ fontSize: 'clamp(40px, 8vw, 96px)' }}>
            TÜRKİYE&apos;NİN<br /><span className="text-red-600">EN BÜYÜK</span> MMA<br />ORGANİZASYONU
          </h2>
          <p className="text-gray-400 text-lg max-w-xl mx-auto mb-8">Her etkinlikte dünyaca ünlü savaşçılar ve nefes kesen karşılaşmalar.</p>
          <Link href="/etkinlikler" className="btn-red text-base px-10 py-4">Tüm Etkinlikleri Keşfet</Link>
        </div>
      </section>
    </div>
  );
