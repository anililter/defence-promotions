  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <section className="relative min-h-[70vh] flex items-end overflow-hidden">
        {event.poster_url ? (
          <Image src={event.poster_url} alt={event.title} fill className="object-cover opacity-40" priority />
        ) : (
        ...
        <div className="container-site relative z-10 pt-32 pb-12">
          <div className="flex items-center gap-2 mb-4">
            <Link href="/etkinlikler" className="text-gray-500 text-xs tracking-widest uppercase hover:text-white transition-colors">Etkinlikler</Link>
          ...
          {event.subtitle && <p className="text-gray-400 text-xl mb-4">{event.subtitle}</p>}
        </div>
      </section>

      {/* Info + Ticket */}
      <section className="border-y border-white/5" style={{ backgroundColor: 'var(--color-dark)' }}>
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-6 justify-between">
            <div className="flex flex-wrap gap-6">
              {event.event_date && (
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-red-600/10 flex items-center justify-center rounded">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#C8102E" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                  </div>
                  <div>
                    ...
                    {formatTime(event.event_date) && <div className="text-gray-500 text-xs">{formatTime(event.event_date)}</div>}
                  </div>
                </div>
              )}
              {(event.venue || event.city) && (
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-red-600/10 flex items-center justify-center rounded">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#C8102E" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
                  </div>
                  <div>
            ...
            {event.ticket_url ? (
              <a href={event.ticket_url} target="_blank" rel="noopener noreferrer" id="ticket-link" className="btn-gold text-sm px-8 py-4 self-start">🎟️ Bilet Al</a>
      ...
      {fights.length > 0 && (
        <section className="py-20">
          <div className="container-site">
            ...
            <h2 className="section-title text-white mb-10">DÖVÜŞLER</h2>
            {mainEvent && (
              <div className="mb-8">
                <h3 className="text-yellow-400 text-xs font-bold tracking-widest uppercase mb-4 flex items-center gap-2">
                  <span className="inline-block w-4 h-0.5 bg-yellow-400" />Ana Maç
                </h3>
            ...
            {undercard.length > 0 && (
              <div>
                <h3 className="text-gray-500 text-xs font-bold tracking-widest uppercase mb-4 flex items-center gap-2">
                  <span className="inline-block w-4 h-0.5 bg-gray-700" />Alt Kart
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {undercard.map(fight => <FightCard key={fight.id} fight={fight} />)}
                </div>
              </div>
            )}
          </div>
      ...
      {event.description && (
        <section className="py-16 border-t border-white/5">
          <div className="container-site max-w-3xl">
            ...
            <div className="text-gray-400 text-base leading-relaxed" dangerouslySetInnerHTML={{ __html: event.description }} />
          </div>
        </section>
      )}
      ...
      {event.ticket_url && (
        <section className="py-16 bg-red-600/5 border-t border-red-600/10">
          <div className="container-site text-center">
            <h2 className="text-white mb-4" style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '48px' }}>BİLETİNİ HEMEN AL</h2>
            <p className="text-gray-500 mb-8">Kontenjan sınırlıdır. Yerini şimdiden garantile.</p>
            <a href={event.ticket_url} target="_blank" rel="noopener noreferrer" className="btn-gold text-base px-12 py-4">🎟️ Bilet Satın Al</a>
          </div>
        </section>
      )}
    </div>
